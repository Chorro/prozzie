...
if __name__ == "__main__":
    invalid_values = [
        # Must not use a directory
        Line('/tmp'),
        # Must not use a not existent/random path
        Line('/tmp/not_exists'),
        ]

    responses_sendline = {
        'Password to decrypt key \\(blank for no password\\):': [Line('')],
        'Private key to use \\(blank for plain http\\)': [
            *invalid_values[:],
            Line(''),
        ],
        'Certificate to export \\(blank for plain http\\)': [
            *invalid_values[:],
            Line(''),
        ],
